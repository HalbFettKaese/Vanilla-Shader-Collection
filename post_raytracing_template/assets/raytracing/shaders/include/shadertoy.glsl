...
vec3 background(vec3 r) {
	...
	float t = GameTime*1200.0;				// add god rays
...
vec3 bgColor(ray camRay) {
    float t = GameTime*1200.0*.04;
...
vec3 render(ray camRay, vec3 mainColor, float depth) {
    // outputs a color
    ...
    vec3 col = bg;
    float d;
    de o = CastRay(camRay, d);

    float fog_fade = smoothstep(0.0, MAX_DISTANCE, depth);

    fog_fade = 1. - pow(1.-fog_fade, 3.);

    bg = mix(mainColor, bg, fog_fade);
